...
export interface PrizeTierData {
  ...
  prizeType: string;
  order: number;
  valueNGN?: number; // Added for consistency with PrizeStructureForm
}
...
export interface PrizeStructureData {
  ...
  description: string;
  isActive: boolean;
  prizes: PrizeTierData[]; // Changed from prizeTiers to prizes for consistency with form
  createdAt: string;
  validFrom: string;
  validTo?: string | null;
}
...
const styles = {
  ...
  infoSection: { marginBottom: "20px", padding: "15px", border: "1px solid #e0e0e0", borderRadius: "8px", backgroundColor: "#e0f7fa" }, // Light cyan background
  prizeBreakdownSection: { marginTop: "15px", padding: "15px", border: "1px solid #dcedc8", borderRadius: "8px", backgroundColor: "#f1f8e9" }, // Light green background
...
const mockPrizeStructures: PrizeStructureData[] = [
  {
    ...
    prizes: [
      { id: 1, name: "Jackpot", value: "N1,000,000", valueNGN: 1000000, quantity: 1, prizeType: "Cash", order: 0 },
      { id: 2, name: "Consolation", value: "N10,000 Airtime", valueNGN: 10000, quantity: 5, prizeType: "Airtime", order: 1 },
    ],
  },
  {
    id: 2,
    ...
    prizes: [
      { id: 3, name: "Grand Weekend Prize", value: "N5,000,000", valueNGN: 5000000, quantity: 1, prizeType: "Cash", order: 0 },
      { id: 4, name: "Weekend Mini", value: "N50,000 Airtime", valueNGN: 50000, quantity: 10, prizeType: "Airtime", order: 1 },
    ],
  },
  {
    id: 3,
    ...
    prizes: [
      { id: 4, name: "Expired Prize", value: "N100", valueNGN: 100, quantity: 1, prizeType: "Cash", order: 0 },
    ],
  },
];

// Store executed draws in session for mock re-run prevention
...
const DrawManagementPage: React.FC = () => {
  ...
  useEffect(() => {
    if (selectedPrizeStructureId) {
      ...
      setDrawResult(null); // Clear previous results when structure changes
      setShowRerunConfirm(false); // Hide rerun confirm if structure changes
      setRerunConfirmText("");
    } else {
  ...
  const handleExecuteDraw = async (isRerun: boolean = false) => {
    ...
    const alreadyRun = executedDraws.find(d => d.drawDate === drawDate && d.prizeStructureId === selectedPrizeStructureId);
    ...
    if (!alreadyRun || isRerun) { // Add to executed only if it's a new run or confirmed re-run
  ...
  const selectedStructureDetails = availablePrizeStructures.find(s => s.id === selectedPrizeStructureId);
        ...
        {canExecuteDraw && showRerunConfirm && (
          <div style={{marginTop: "15px"}}>
            <input 
              ...
              style={{...styles.input, width: "calc(100% - 220px)", marginRight: "10px", display: "inline-block"}}
            />
