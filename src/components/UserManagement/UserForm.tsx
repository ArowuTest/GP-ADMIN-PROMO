...
const UserForm: React.FC<UserFormProps> = ({ user, onSave, onCancel }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Prevent deactivating the primary superadmin (if such logic is needed here or in UserListComponent)
    if (isEditing && user?.id === 1 && user?.username === "superadmin" && !formData.isActive) {
        setError("The primary superadmin cannot be deactivated.");
        // Potentially revert formData.isActive or handle in onSave
        return;
    }
    ...
    <form onSubmit={handleSubmit} style={styles.form}>
      ...
      <div style={styles.checkboxLabel}>
        <input
          ...
          disabled={isEditing && user?.id === 1 && user?.username === "superadmin"} // Prevent deactivating primary superadmin
        />
